...
namespace POLA_MMS.Member_UI
{
    public partial class frmMember_Recharge : Form
    {
        ...
        {
            List<Deposit> lstDeposit = new DepositManager().SelectList(this.m_strMemberId, this.cboDate.Year, this.cboDate.Month, this.cboDate.Day);
        ...
        private void cmnuDeposit_Delete_Click(object sender, EventArgs e)
        {
            if (this.dgvDeposit.CurrentRow != null)
            {
                ...
                {
                    DepositManager objDepositM = new DepositManager();
                    Deposit objDeposit = objDepositM.GetDeposit(decimal.Parse(iDepositId));
                    if (objDepositM.DeleteDeposit(int.Parse(objDeposit.ID.ToString())))
                    {
                        this.LoadMemberInfo();
                        this.LoadRechargeList();
                    }
        ...
        private void dgvDeposit_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            if (e.Button == MouseButtons.Right)
            {
                if (rows.RowIndex > -1 && rows.ColumnIndex > -1)
                {
                    ...
                    Deposit objDeposit = new DepositManager().GetDeposit(int.Parse(this.dgvDeposit.CurrentRow.Cells[0].Value.ToString()));
        ...
        {
            Common.ShowRows_DataGridView_RowPostPaint(this.dgvDeposit, sender, e);
        }

        /// <summary>刷新</summary>
        private void cmnuDeposit_Refresh_Click(object sender, EventArgs e)
        {
            new MemberManager().UpdateBalance(this.m_strMemberId);
            this.LoadMemberInfo();
            this.LoadRechargeList();
        }
    }
