...
namespace POLA_MMS.Member_UI
{
    public partial class frmMember : Form
    {
        ...
        /// <param name="ChangeCardID">是否更改会员卡号</param>
        public frmMember(string memberid,bool ChangeCardID)
        {
            InitializeComponent();
        ...
        private void cboLevel_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (this.m_bRun)
            {
                Card objCard = new CardManager().Select(int.Parse(this.cboLevel.SelectedValue.ToString()));
        ...
        {
            Member objMember = new MemberManager().LoadMemberInfo(this.m_strMemberId);
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            objMember.Password = Common.EncryptMD5(this.txtPassword.Text.Trim());
            ...
            string strTempId = new MemberManager().ExistsMemberCard(objMember.ID);
            if (this.m_strMemberId != "")
            {
                objMember.ID = this.m_strMemberId;
                if (this.txtCard.Enabled == false)
                {
                    new MemberManager().UpdateMember(objMember);
                }
                else
                {
                    ...
                    objMember.ID = txtCard.Text;
                    new MemberManager().UpdateMemberID(objMember,this.m_strMemberId);
                }
            }
            else
            {
                ...
                try
                {
                    new MemberManager().InsertMember(objMember);
                    new MemberManager().UpdateLastTime(objMember.ID, System.DateTime.Now);
                }
                catch
                {
                    MessageBox.Show("信息不完善，请完善信息","提示", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                }
