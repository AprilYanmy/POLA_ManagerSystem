...
namespace POLA_MMS.Report_UI
{
    public partial class frmReport_Month : WinFormsUI.Docking.DockContent
    {
        ...
        private void LoadMoneyOfMonth()
        {
            ...
            List<SubjectMonth> lstMonthTable = new SubjectMonthManager().SelectList(iYear, iMonth);
            this.dgvMonth.AutoGenerateColumns = false;
            this.dgvMonth.Rows.Clear();
            decimal dIncome = 0;
            decimal dExpend = 0;
            decimal dProfit = 0;
            foreach (SubjectMonth objMonth in lstMonthTable)
            {
        ...
        {
            Common.ShowRows_DataGridView_RowPostPaint(this.dgvMonth, sender, e);
        }
    }
}
