...
namespace POLA_MMS.UI_Employee
{
    public partial class frmEmployee_List : WinFormsUI.Docking.DockContent
    {
        ...
        private void LoadPostList()
        {
            ...
            List<Post> lstPost = new PostManager().SelectList();
        ...
        private void LoadEmployeeList()
        {
            ...
            List<Employee> lstEmployee = new EmployeeManager().SelectList(postId, i);
        ...
        private void btnDel_Click(object sender, EventArgs e)
        {
            ...
            {
                int iEmpId = int.Parse(this.dgvEmployee.CurrentRow.Cells[0].Value.ToString());
                if (MessageBox.Show("确实要删除选定的员工吗？", "消息", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    int res = new EmployeeManager().DeleteEmployee(iEmpId);
                    if (res == 1)
                    {
        ...
        {
            int iEmpId = int.Parse(this.dgvEmployee.CurrentRow.Cells[0].Value.ToString());
            frmEmployee_Pays objfrmPaysEmployee = new frmEmployee_Pays(iEmpId);
            objfrmPaysEmployee.ShowDialog();
        }

        private void dgvEmployee_RowPostPaint(object sender, DataGridViewRowPostPaintEventArgs e)
        {
            Common.ShowRows_DataGridView_RowPostPaint(this.dgvEmployee, sender, e);
        }
